...
def ppo_loop(args):
    ...
    scores_list = [0]
    env = ZorkEnv("zork1.z5", max_repeat=3  0)
    # Sample trajetories
    for episode in range(args.max_episodes):
        ...
        while not env.terminal and step < args.max_epochs:
            ...
            if thinking_flag:
                value = critic(location, [], action) #No inventory info
        ...
        for _ in tqdm(range(ppo_epochs)):
            ...
            for i in tqdm(range(len(transitions["thinking"]))):
                ...
                if thinking_flag:
                    ...
                    target = returns[j]
                    critic_loss = critic.update(location, [], action, target, lr=args.critic_lr) # No inventory info
