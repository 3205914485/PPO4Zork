...
def build_actor_prompt(history, current, memory):
    ...
    if history:
        ...
        prompt += "\n"
    prompt += "Memory:" + memory + "Do not go to visited rooms again. Need to follow the candidates actions to explore more new places."
...
def generate_action(tokenizer, actor, prompt, max_length=2048):
    with torch.no_grad():
        ...
        full_input_ids = tokenizer(full_input, return_tensors="pt").input_ids
        outputs = actor.model(input_ids=full_input_ids)
        logits = outputs.logits[:, :-1, :]
        log_probs = torch.log_softmax(logits, dim=-1)

        generated_ids = tokenizer(generated_text, return_tensors="pt").input_ids[:, 1:]
...
def ppo_loop(args):
    ...
    for episode in range(args.max_episodes):
        ...
        step = 0
        transitions = {"prompts": [], "locations": [], "inventories": [], "actions": [], "generates": [], "rewards": [], "logprobs": [], "values": [], "thinking": []}
        ...
        for traj, score in tqdm(sampled_trajectories):
            ...
            for i in range(len(thinking_flags)):
                ...
                if thinking_flag:
                    ...
                    # --- critic ---
                    critic_loss = critic.update(location, [], 'N', target, lr=args.critic_lr)
